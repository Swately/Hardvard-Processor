#!/usr/bin/env python3
"""run_jitter.py -- assemble the jitter experiment and verify the instrument.
...
WORK = os.path.join(SIM, "work_jitter")
...
def main():
    ...
    r = run([sys.executable, os.path.join(ROOT, "tools", "pasm.py"),
             os.path.join(ROOT, "sw", "jitter.s"), "-o", image])
    ...
    files = ([os.path.join(ROOT, "primitives", "mem_pkg.vhd")]
             ...
             + [os.path.join(SIM, "tb_jitter.vhd")])
    for f in files:
    ...
    print("\n=== ELABORATE ===")
    r = run([ghdl, "-e", "--std=08", f"--workdir={WORK}", "tb_jitter"])
    if r.returncode != 0:
    ...
    print("\n=== PLAY ===")
    r = run([ghdl, "-r", "--std=08", f"--workdir={WORK}", "tb_jitter",
             "--stop-time=1000ms", "--stop-delta=2000"])
